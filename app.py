...
def add_order_to_user(username, order_id):
    users = load_data(USERS_FILE)
    if username in users:
...
def checkout():
    ...
    if request.method == 'POST':
        ...
        add_order_to_user(session['user'], order_id)
    ...
    countries = settings.get('countries', ['United States', 'Canada', 'United Kingdom', 'Australia', 'Germany', 'France', 'Other'])
...
def admin_dashboard():
    ...
    for oid in orders:
        orders[oid] = fix_order_items(orders[oid])
...
def admin_delete_order(order_id):
    ...
    if order_id in orders:
        ...
        for username, user_data in users.items():
            if 'orders' in user_data and order_id in user_data['orders']:
...
if __name__ == '__main__':
    print("=" * 60)
    print("🛍️  AFG SHOP - COMPLETE E-COMMERCE")
    print("=" * 60)
    print("🔗 Website URL: http://0.0.0.0:5000")
